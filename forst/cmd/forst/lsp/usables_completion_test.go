...
func TestUsablesWiringCompletion_contractKeys(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	if err := os.WriteFile(filepath.Join(dir, "go.mod"), []byte("module usables_compl\n\ngo 1.23\n"), 0o644); err != nil {
		t.Fatal(err)
	}
...
func TestUsablesWiringCompletion_implTypesAfterColon(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	if err := os.WriteFile(filepath.Join(dir, "go.mod"), []byte("module usables_compl\n\ngo 1.23\n"), 0o644); err != nil {
		t.Fatal(err)
...
func TestUsablesUseCompletion_contractAfterColon(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	if err := os.WriteFile(filepath.Join(dir, "go.mod"), []byte("module usables_use\n\ngo 1.23\n"), 0o644); err != nil {
		t.Fatal(err)
	}
