...
func usablesWiringCompletionItems(ctx *forstDocumentContext, pos LSPPosition, prefix string) []LSPCompletionItem {
	...
	if cur.afterColon && cur.fieldKey != "" {
		impls := ctx.TC.UsableImplTypeNamesForContract(cur.fieldKey)
		return completionItemsFromStrings(impls, prefix, "2")
	}
	keys := ctx.TC.KnownUsableRootKeys()
...
func effectiveUsableContractLabels(ctx *forstDocumentContext, pos LSPPosition) []string {
	if ctx == nil || ctx.TC == nil {
		return nil
	}
	line1 := pos.Line + 1
	...
	if len(chain) > 0 {
		labels, err := ctx.TC.EffectiveAmbientKeyLabels(chain)
	...
	return ctx.TC.KnownUsableRootKeys()
}

func usablesUseCompletionItems(ctx *forstDocumentContext, pos LSPPosition, prefix string) []LSPCompletionItem {
	if ctx == nil || ctx.TC == nil {
		return nil
	}
	...
	var labels []string
	if afterColon {
		labels = ctx.TC.KnownUsableRootKeys()
	} else {
		labels = effectiveUsableContractLabels(ctx, pos)
	}
