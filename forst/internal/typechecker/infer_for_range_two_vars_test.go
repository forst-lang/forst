...
func TestRangeTypesForTwoVars_arrayMapString(t *testing.T) {
	...
	key, val, err = tc.rangeTypesForTwoVars(ast.TypeNode{Ident: ast.TypeString}, ast.FakeSpan())
	if err != nil {
		t.Fatal(err)
	}
...
func TestRangeTypesForTwoVars_unsupported(t *testing.T) {
	t.Parallel()
	tc := New(logrus.New(), false)
	_, _, err := tc.rangeTypesForTwoVars(ast.TypeNode{Ident: ast.TypeFloat}, ast.FakeSpan())
	if err == nil {
