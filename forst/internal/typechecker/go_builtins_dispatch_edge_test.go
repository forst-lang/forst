...
func TestInferBuiltinArgType_missingIndex(t *testing.T) {
	...
	_, err := tc.inferBuiltinArgType([]ast.ExpressionNode{}, 0, nil, ast.FakeSpan())
	if err == nil {
...
func TestCheckBuiltinFunctionCall_dispatchKindWithoutHandlerReturnsInternalError(t *testing.T) {
	...
	_, err := tc.checkBuiltinFunctionCall(fn, nil, nil, ast.FakeSpan())
	if err == nil {
...
func TestCheckBuiltinFunctionCall_capRejectsString(t *testing.T) {
	...
	_, err := tc.checkBuiltinFunctionCall(fn, []ast.ExpressionNode{ast.StringLiteralNode{Value: "abc"}}, nil, ast.FakeSpan())
	if err == nil {
