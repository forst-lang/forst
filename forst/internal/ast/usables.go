...
// IsUsablesWiringRoot reports whether fnIdent is a wiring root (main or Test* with *testing.T).
// When paramTypes is empty for a Test* ident, the function is treated as a test wiring root.
func IsUsablesWiringRoot(fnIdent Identifier, paramTypes []TypeNode) bool {
	if fnIdent == "main" {
		return true
	}
