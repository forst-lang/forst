...
func TestIsProvidersWiringRoot(t *testing.T) {
	if !IsProvidersWiringRoot("main", nil) {
	...
	if !IsProvidersWiringRoot("TestFoo", []TypeNode{tParam}) {
		t.Fatal("Test* with *testing.T is wiring root")
	}
	if IsProvidersWiringRoot("outer", nil) {
		t.Fatal("outer is not wiring root")
	}
	if !IsProvidersWiringRoot("TestUnknown", nil) {
