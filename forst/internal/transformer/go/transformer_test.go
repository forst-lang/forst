...
func TestEnsureErrorReturnType_UsesNamedReturnType(t *testing.T) {
	// This test reproduces the bug where the error path of an ensure statement
	// returns the wrong zero value type (e.g., User{} instead of CreateUserResponse{})
	// when using shape guards (is (user User) Valid)
	...
	// Create the CreateUserRequest type
	...
	// Create the CreateUserResponse type
	...
	userAssign := ast.AssignmentNode{
		LValues: []ast.VariableNode{{Ident: ast.Ident{ID: ast.Identifier("user")}}},
		RValues: []ast.ExpressionNode{
			ast.ShapeNode{
				Fields: map[string]ast.ShapeFieldNode{
					"id":    ast.MakeStructField(ast.StringLiteralNode{Value: "123"}),
					"name":  ast.MakeStructField(ast.VariableNode{Ident: ast.Ident{ID: ast.Identifier("input.name")}}),
					"age":   ast.MakeStructField(ast.VariableNode{Ident: ast.Ident{ID: ast.Identifier("input.age")}}),
					"email": ast.MakeStructField(ast.VariableNode{Ident: ast.Ident{ID: ast.Identifier("input.email")}}),
				},
			},
		},
		IsShort: true,
	...
	// Create a boolean variable that's always false to trigger the error path
	alwaysFalse := ast.AssignmentNode{
		LValues: []ast.VariableNode{{Ident: ast.Ident{ID: ast.Identifier("isValid")}}},
		RValues: []ast.ExpressionNode{ast.BoolLiteralNode{Value: false}},
		IsShort: true,
	}

	// Simulate: ensure user is Valid (will always fail due to alwaysFalse)
	ensureStmt := ast.EnsureNode{
		Variable: ast.VariableNode{Ident: ast.Ident{ID: ast.Identifier("user")}},
		Assertion: ast.AssertionNode{
			BaseType: typeIdentPtr("User"),
	...
	returnStmt := ast.ReturnNode{
		Values: []ast.ExpressionNode{
			ast.ShapeNode{
				Fields: map[string]ast.ShapeFieldNode{
					"user":       ast.MakeStructField(ast.VariableNode{Ident: ast.Ident{ID: ast.Identifier("user")}}),
	...
	}, []ast.Node{userAssign, alwaysFalse, ensureStmt, returnStmt})

	// Add back the shape guard: is (user User) Valid
	validGuard := ast.TypeGuardNode{
		Ident: ast.Identifier("Valid"),
		Subject: ast.SimpleParamNode{
			Ident: ast.Ident{ID: ast.Identifier("user")},
	...
	// Create type checker and register types
	tc := typechecker.New(nil, false)
	// Typecheck all top-level nodes
	err := tc.CheckTypes([]ast.Node{userType, createUserRequestType, createUserResponseType, validGuard, fn})
	if err != nil {
		t.Fatalf("Typechecking failed: %v", err)
	}

	// Debug: Check what's in tc.Defs
	t.Logf("=== DEBUG: tc.Defs contents ===")
	for key, def := range tc.Defs {
		t.Logf("Key: %s, Type: %T, Value: %+v", key, def, def)
	}
	...
	// The error path should return CreateUserResponse{}, not User{}
	if !strings.Contains(resultStr, "return CreateUserResponse{}") {
		t.Errorf("Expected error path to return CreateUserResponse{}, got: %s", resultStr)
	}

	// Should not return User{} in the error path
	if strings.Contains(resultStr, "return User{}") {
		t.Errorf("Error path incorrectly returns User{} instead of CreateUserResponse{}: %s", resultStr)
	}
