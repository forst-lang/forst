...
func TestTransformFunctionCallWithShapeLiteralArgument_UndefinedTypeBug(t *testing.T) {
	// Setup minimal typechecker and transformer
	tc := typechecker.New(nil, false)
	tr := New(tc, nil)
	...
	// DO NOT register the inferred type in tc.Defs (simulate the bug)
	// This forces the transformer to use a hash-based type that is not defined
	...
	// Check that the generated Go code uses a type that is NOT defined in tc.Defs
	callExpr, ok := stmt.(*goast.ExprStmt)
	if !ok {
		t.Fatalf("Expected ExprStmt, got %T", stmt)
	}
	...
	// The bug: the type should NOT be defined in tc.Defs
	if _, exists := tc.Defs[ast.TypeIdent(ident.Name)]; exists {
		t.Errorf("Type %q used in composite literal IS defined in tc.Defs, but should not be (this is the bug)", ident.Name)
	}
}
