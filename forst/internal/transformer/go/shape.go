...
func (t *Transformer) transformShapeFieldType(field ast.ShapeFieldNode) (*goast.Expr, error) {
	if field.Type != nil {
		...
		if !typechecker.IsGoBuiltinType(string(field.Type.Ident)) && field.Type.Ident != ast.TypeString && field.Type.Ident != ast.TypeInt && field.Type.Ident != ast.TypeFloat && field.Type.Ident != ast.TypeBool && field.Type.Ident != ast.TypeVoid {
			if def, ok := t.TypeChecker.Defs[field.Type.Ident]; ok {
...
func (t *Transformer) transformShapeType(shape *ast.ShapeNode) (*goast.Expr, error) {
	...
	for _, name := range ast.ShapeFieldNamesInOrder(shape.Fields, shape.FieldOrder) {
		...
		goFieldName := t.goShapeDataFieldName(name, field)
		goField := &goast.Field{
			Type: *fieldType,
		}
		if !field.Embedded {
		...
		if field.Tag != "" {
			goField.Tag = &goast.BasicLit{Kind: token.STRING, Value: "`" + field.Tag + "`"}
		} else if t.ExportReturnStructFields && !field.Embedded {
