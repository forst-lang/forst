...
func writeTestingErrorsImports(b *strings.Builder, functions []FunctionSignature) {
	valueNames := []string{"ForstTestServerFailed", "InvokeRejected"}
	exclude := map[string]struct{}{
		"ForstTestServerFailed": {},
		"InvokeRejected":        {},
	}
	typeNames := collectFailureTypeImports(functions)
	var typeOnly []string
	for _, name := range typeNames {
		if _, skip := exclude[name]; skip {
			continue
		}
		typeOnly = append(typeOnly, "type "+name)
	}
	parts := append([]string{}, valueNames...)
	parts = append(parts, typeOnly...)
...
func EmitPackageEffectDTS(m ModuleEmit, npmPackageName string) string {
	...
	writeErrorsTypeImport(&b, m.Functions, "../errors.js")
