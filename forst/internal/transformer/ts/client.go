...
func (t *TypeScriptTransformer) generateTypesFile() {
	...
	if len(t.Output.Functions) > 0 {
		...
		functionSignatures := make([]string, len(t.Output.Functions))
		for i, function := range t.Output.Functions {
			functionSignatures[i] = function.ToString()
		}
		lines = append(lines, strings.Join(functionSignatures, "\n\n"))
		lines = append(lines, "")
	}
