...
func TestReload_parentOwnedHostSurvivesGroupKill(t *testing.T) {
	...
	nodePID := bridgert.ReadHostMarkerPID(root)
	if nodePID <= 0 {
	...
	t.Cleanup(func() { _ = bridgert.ShutdownForTest() })
	if _, err := bridgert.GetClient(); err != nil {
...
func TestHostOrchestrator_shutdownTerminatesSpawnedHost(t *testing.T) {
	...
	dir := t.TempDir()
	_, readyPath, err := bridgert.ResolveHostSocketPath(dir, "")
	if err != nil {
		t.Fatal(err)
	}
	...
	pid := bridgert.ReadHostMarkerPID(dir)
	if pid <= 0 {
...
func loadHostModeConfig(root string) (*ftconfig.Config, error) {
	if err := os.WriteFile(filepath.Join(root, "ftconfig.json"), []byte(`{"bridge":{"hostMode":true,"args":["x"]}}`), 0o644); err != nil {
		return nil, err
	}
