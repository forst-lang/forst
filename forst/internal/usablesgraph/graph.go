...
// Graph is the shared Usables call graph for checker, discovery JSON, and LSP (ADR-036).
type Graph struct {
...
// SetDirect records Usables introduced by use sites in fn.
...
// AddIntraCall records a call edge with ambient keys at the call site.
func (g *Graph) AddIntraCall(caller, callee ast.Identifier, ambient AmbientSnapshot) {
	g.intraEdges = append(g.intraEdges, IntraPackageEdge{
		Caller:  caller,
		Callee:  callee,
		Ambient: cloneAmbient(ambient),
	})
...
func (g *Graph) AddModuleCall(edge ModuleCallEdge) {
	edge.Ambient = cloneAmbient(edge.Ambient)
	g.moduleEdges = append(g.moduleEdges, edge)
}

// ComputeIntraFixedPoint runs intra-package propagation into g.slots.
func (g *Graph) ComputeIntraFixedPoint(satisfies SatisfiesAmbient) {
	PropagateIntraPackageFixedPoint(g.slots, g.direct, g.intraEdges, satisfies)
}

// Slots returns inferred Usables for fn after fixed-point computation.
func (g *Graph) Slots(fn ast.Identifier) []Slot {
	return g.slots[fn]
}

// AllSlots returns a copy of all function Usables in the graph.
...
// Invalidate returns transitive intra-package callers of fn whose Usables may change
// when fn's direct use sites or Usables(g) change.
...
// ModuleGraph holds per-package Usables for cross-package fixed-point propagation.
...
func (m *ModuleGraph) ComputeFixedPoint(satisfies SatisfiesAmbient) {
...
func cloneAmbient(src AmbientSnapshot) AmbientSnapshot {
	if len(src) == 0 {
		return nil
	}
	out := make(AmbientSnapshot, len(src))
	for k, v := range src {
		out[k] = v
	}
	return out
