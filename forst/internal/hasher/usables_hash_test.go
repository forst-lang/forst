...
func TestNodeHash_ToUsablesIdent_prefix(t *testing.T) {
	h := HashSortedStrings("Logger", "Clock")
	id := h.ToUsablesIdent()
	if !stringsHasPrefix(id, "Usables_") {
		t.Fatalf("ToUsablesIdent() = %q, want Usables_ prefix", id)
	}
	if len(id) <= len("Usables_") {
		t.Fatalf("ToUsablesIdent() too short: %q", id)
	}
}
