...
func skipUnlessBridgeHostTransport(t *testing.T, host ftconfig.JSHost) {
	t.Helper()
	if host == ftconfig.JSHostDeno {
		return
	}
...
func writeBridgeHostFtconfig(t *testing.T, root string, host ftconfig.JSHost, binary string, format ftconfig.LegacyModuleFormat) {
	t.Helper()
	cfgJSON := fmt.Sprintf(`{
  "javascript": {
    "host": %q,
