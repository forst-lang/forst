...
func setupRemixServeIntegrationRoot(t *testing.T) string {
	t.Helper()
	root, err := filepath.Abs(filepath.Join(repoRoot(t), "examples", "in", "rfc", "bridge-interop", "remix-serve"))
	if err != nil {
		t.Fatal(err)
	}
...
func linkNodeRuntimeFromRepo(t *testing.T, root string) {
	...
	if _, err := os.Stat(filepath.Join(src, "dist", "host.js")); err != nil {
		t.Skipf("/runtime not built (run task build:runtime): %v", err)
	}
	dstDir := filepath.Join(root, "node_modules", "@forst")
	if err := os.MkdirAll(dstDir, 0o755); err != nil {
		t.Fatal(err)
	}
	dst := filepath.Join(dstDir, "runtime")
	if err := os.Symlink(src, dst); err != nil {
		t.Fatal(err)
	}
}
...
func hostModulePath(t *testing.T) string {
	t.Helper()
	path := filepath.Join(repoRoot(t), "packages", "runtime", "dist", "host.js")
	if _, err := os.Stat(path); err != nil {
		t.Skipf("host.js not built (run task build:runtime): %v", err)
	}
