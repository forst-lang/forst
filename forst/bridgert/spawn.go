...
const (
	envNodeHost       = "FORST_BRIDGE_HOST"
	envNodeHostLeader = "FORST_BRIDGE_HOST_LEADER"
	envNodeSocket     = "FORST_BRIDGE_SOCKET"
	envNodeHostReady  = "FORST_BRIDGE_HOST_READY"
	envNodeAttachOnly = "FORST_BRIDGE_ATTACH_ONLY"
)

// EnvNodeAttachOnly is FORST_BRIDGE_ATTACH_ONLY; see block comment above.
...
func BuildHostSpawnCommand(in HostSpawnInput) (HostSpawnCommand, error) {
	if len(in.ShimArgs) == 0 {
		return HostSpawnCommand{}, fmt.Errorf("node runtime: hostMode requires non-empty bridge.args")
	}
