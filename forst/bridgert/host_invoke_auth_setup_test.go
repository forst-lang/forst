...
func TestEnsureEmbeddedHostInvokeAuthRelay_earlyReturns(t *testing.T) {
	tests := []struct {
		...
		{name: "skip node host", cfg: &ftconfig.Config{Bridge: ftconfig.BridgeConfig{HostMode: true}, Server: ftconfig.ServerConfig{Embedded: true}}, env: map[string]string{EnvSkipNodeHost: "1"}},
		{name: "auth off", cfg: &ftconfig.Config{Bridge: ftconfig.BridgeConfig{HostMode: true}, Server: ftconfig.ServerConfig{Embedded: true}}, env: map[string]string{envInvokeAuth: "off"}},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
...
func TestEnsureEmbeddedHostInvokeAuthRelay_respectsInheritedSpawnHandoff(t *testing.T) {
	...
	cfg := &ftconfig.Config{
		Bridge:   ftconfig.BridgeConfig{HostMode: true},
