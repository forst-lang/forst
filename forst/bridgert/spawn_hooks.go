...
func resolveHostInterpreter(boundaryRoot string, host ftconfig.BridgeHost, configured string) (string, error) {
	name := string(host)
	if configured != "" {
...
func sanitizeNodeOptionsForHost(env []string, host ftconfig.BridgeHost) []string {
	if host == ftconfig.BridgeHostNode {
		return env
	}
