...
Import shape, subpaths, and layout: [Generate a TypeScript client](/interop/invoke/generate-types). Calling functions at runtime: [Call Forst over HTTP](/interop/invoke/call-forst).

**Monorepos:** put `postinstall` on the workspace package that owns `ftconfig.json`, not the repo root unless that root is the boundary. See [Generate a TypeScript client § Monorepos](/interop/invoke/generate-types#monorepos-and-multiple-forst-projects).
