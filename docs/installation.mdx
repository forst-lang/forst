...
COPY . .
RUN forst generate . \
 && forst build -root . -o .forst/build/invoke --goos linux --goarch amd64 -- ./app/main.ft

# runtime stage
FROM node:22-bookworm-slim
WORKDIR /app
COPY --from=forst-build /app/.forst/client /app/.forst/client
COPY --from=forst-build /app/.forst/build/invoke /app/.forst/build/invoke
ENV FORST_BOUNDARY_ROOT=/app
CMD ["/app/.forst/build/invoke/bin/main"]
```

Adjust paths for your layout. Read `manifest.json` `binary` for the linked executable name. The runtime stage needs Node (for `@forst/gen`) and the built program binary, not the Forst compiler or Go toolchain. With `node.hostMode`, the Go binary is the parent process: it runs embedded invoke and spawns the Node host child.
