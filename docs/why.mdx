...
While backend development covers a wide surface, three central issues affect everyone:

- **Typed boundaries**: request and response shapes at the API edge need validation before bad data reaches domain logic
- **Developer experience**: everyday work needs fast feedback and low boilerplate
- **Production runtime**: production deploys need ops that hold when traffic spikes as complexity grows
...
| --- | --- | --- | --- |
| Production runtime | JS (Node, Bun, Deno) | Native (`go build`) | [Go (compiled from `.ft`)](/interop/go) |
...
| Server and client types from one source | N/A | Manual / separate | [`forst generate`](/interop/typescript) |
| Error model | Exceptions (`try`/`catch`) | `(T, error)` values | [`ensure`, `Result`, nominal errors`](/language/errors-and-result) |
| Type narrowing | Discriminated unions | Type assertions | [`is` / `ensure`](/language/ensure-and-narrowing) |
| Go module ecosystem | No | Native | [Import Go packages](/interop/go) |
