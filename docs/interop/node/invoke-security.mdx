...
When `node.hostMode` is enabled, the node host may start before the embedded
invoke server is ready. The dev parent relays auth from the go child
(`FORST_INVOKE_AUTH_FD`) to the host shim over `FORST_INVOKE_AUTH_RECV_FD` so
the host never reads a disk token file.
