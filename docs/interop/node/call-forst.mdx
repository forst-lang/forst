...
<Steps>
  ...
  <Step title="Run a server">
    ...
    **Production** — linked program binary for slim container images:

    ```bash
    npx forst generate .
    npx forst build -root . -o .forst/build/invoke -- ./main.ft
    FORST_BOUNDARY_ROOT=/app /app/.forst/build/invoke/bin/main
    ```

    With `node.hostMode`, the same binary runs embedded invoke and spawns the Node host as a child. Set `FORST_INVOKE_ONLY=1` on the same binary when Node runs separately and only invoke is needed in-process.
