...
modes. See [Generate a TypeScript client](/interop/node/generate-types) and
...
`Content-Type: application/json` is always set. Your headers are merged on top.
...
`allowSpawn: true` and `transport: "dev"` are reserved for a future release. Today the client always connects over HTTP even when those flags are set.
...
```typescript Promise
import { isInvokeFailure } from "@forst/gen/invoke";
import { VerifyPassword } from "@forst/gen/auth";
...
try {
  ...
  switch (error._tag) {
    case "@forst/gen/InvokeRejected":
      return badRequest(error.serverError);
    case "@forst/gen/InvokeBaseUrlMissing":
    case "@forst/gen/InvokeUnreachable":
...
const program = VerifyPassword(input).pipe(
  Effect.catchTag("@forst/gen/InvokeRejected", (error) =>
    Effect.succeed(badRequest(error.serverError))
  ),
  Effect.catchTag("@forst/gen/InvokeUnreachable", () =>
...
| --- | --- | --- |
| `@forst/gen/InvokeRejected` | Server returned `success: false` | Map `serverError` to a client response or retry policy |
| `@forst/gen/InvokeHttpFailure` | Non-2xx HTTP status | Check `status` and `responseText`, fix URL or server |
| `@forst/gen/InvokeTimedOut` | Exceeded `timeoutMs` or abort signal | Raise timeout, fix slow handler, or surface a 504 |
| `@forst/gen/InvokeUnreachable` | Connection refused or DNS failure | Confirm the binary or `forst dev` is listening |
| `@forst/gen/InvokeBaseUrlMissing` | No base URL and spawn is forbidden | Set `FORST_BASE_URL` (required in production) |
| `@forst/gen/InvokeStreamAborted` | NDJSON stream ended early or a row failed to parse | Retry stream, or fix the streaming handler |
| `@forst/gen/ContractVersionMismatch` | Client and server contract versions disagree | Upgrade client and server together |

Import invoke helpers from `@forst/gen/invoke` (`InvokeRejected`, `isInvokeFailure`, …). Class names stay short; `_tag` strings are namespaced with your npm package name so they cannot collide with domain errors or Effect service tags.
...
import { PlayMove } from "@forst/tictactoe/main";

try {
  await PlayMove({ state, row: 1, col: 2 });
} catch (error) {
...
const program = PlayMove(req).pipe(
  Effect.catchTag("CellTaken", (e) => Effect.succeed({ row: e.row, col: e.col }))
);
```
...
```typescript Promise
import { InvokeRejected } from "@forst/gen/invoke";
import { VerifyPassword } from "@forst/gen/auth";
