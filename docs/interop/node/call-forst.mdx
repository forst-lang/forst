...
and a separate local token file when authentication is enabled. Use the Forst
client or sidecar for `POST /invoke`, because authenticated RPC requests require
a fresh challenge and HMAC proof. For a manual smoke test, call the
unauthenticated health endpoint:

```bash
curl -s http://127.0.0.1:6321/health
```
