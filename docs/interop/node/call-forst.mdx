...
<Steps>
  ...
  <Step title="Run a server">
    ...
    **Production** — linked program binary for slim container images. Enable embedded invoke in `ftconfig.json` before building:
    ...
    With `node.hostMode`, the same binary runs your Forst entry and starts the Node app as a child. Set `FORST_SKIP_NODE_HOST=1` on the same binary when Node runs separately and the binary should not spawn the Node host.
