...
```typescript Promise
import { isInvokeFailure } from "@forst/gen/$errors";
import { VerifyPassword } from "@forst/gen/auth";
...
Import invoke helpers from `@forst/gen/$errors` (`InvokeRejected`, `isInvokeFailure`, …). Class names stay short. Built-in invoke, harness, and unknown-failure `_tag` strings use the `@forst/errors/` prefix. Domain error tags are namespaced with your npm package name (for example `@forst/tictactoe/CellTaken`).
...
```typescript Promise
import { InvokeRejected } from "@forst/gen/$errors";
import { VerifyPassword } from "@forst/gen/auth";
