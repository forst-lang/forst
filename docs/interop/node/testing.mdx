...
```typescript
import { InvokeTimedOut } from "@forst/gen/$errors";
import { withForstTestScope } from "@forst/gen/$testing";
import { VerifyPassword } from "@forst/gen/auth";
...
expect(await Echo({ message: "hi" })).toEqual({
  echo: "hi",
  timestamp: 42,
});
```
...
invoke server, and points the default client at it. Flat imports keep working.
