...
Promise mode uses the same tags with `isInvokeFailure` from `@forst/gen/$errors` and a `switch` on `_tag`. See [Call Forst from Node](/interop/node/call-forst#failures).
