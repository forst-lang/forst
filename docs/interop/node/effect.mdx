...
const program = VerifyPassword(input).pipe(
  Effect.catchTag("@forst/gen/InvokeRejected", (e) =>
...
Promise mode uses the same tags with `isInvokeFailure` from `@forst/gen/invoke` and a `switch` on `_tag`. See [Call Forst from Node](/interop/node/call-forst#failures).
...
await Effect.runPromise(
  Echo({ message: "hi" }).pipe(Effect.provide(ForstTestServerLayer()))
);
```

For Vitest or Jest hooks, use `makeForstTestServer()` and call `dispose()` in `afterAll`. See [Testing](/interop/node/testing#call-a-real-forst-server).
