...
`import node` tells Forst that this path points to a Node module. The compiler
...
one. Calls stay on the same machine. If your program does not use `import node`,
...
Forst only loads modules that use `import node`. Plain imports never start the
Node bridge.
...
| --- | --- | --- |
| File name | `import node "./legacy/payment"` | `payment` |
| Alias | `import node checkout "./legacy/api/checkout"` | `checkout` |
...
```bash
npx forst build -root ./my-service -o .forst/build/program -- ./cmd/api/main.ft
...
```bash
FORST_BOUNDARY_ROOT=/app /app/.forst/build/program/bin/main
```

Your deployed app needs Node, `tsx`, and `@forst/node-runtime` when any reachable
Forst package uses `import node`. This applies to the whole program. For example,
if `auth` imports TypeScript and `main` imports `auth`, the final app needs Node.
...
Most projects should use bootstrap mode. It starts a small Node process for Forst
...
Set `FORST_INVOKE_ONLY=1` on the same binary when only embedded invoke should run in-process.
...
reachable `import node` stays a normal Go binary with no Node requirement.
...
| `true` | Node bridge and embedded allowlist | Node, `tsx`, and `@forst/node-runtime` are required |
...
The requirement follows imports across packages. A direct `import node` in
...
missing runtime package, a module that was not imported with `import node`, or a
...
`import node` line does not need to appear in `main.ft`. It may come from a
