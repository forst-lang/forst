...
User TypeScript loads only via controlled `import()` inside the bootstrap, triggered by an allowed RPC. Bootstrap RPC uses a local socket; runtime logs go to stderr and child stdout/stderr are forwarded to the Forst parent.
