...
Working reference: [`examples/in/rfc/node-interop/remix-serve`](https://github.com/forst-lang/forst/tree/main/examples/in/rfc/node-interop/remix-serve) (`vite.config.ts`, `task example:node-interop-remix-serve:standalone-e2e` in CI).
...
Edge runtimes (Cloudflare Workers, Vercel Edge, Deno Deploy) and browser bundles are **not** supported by the generated client today. Run `forst generate` in a Node boundary and call Forst from a supported backend. See [Call Forst from Node § Runtime support](/interop/node/call-forst#runtime-support).
