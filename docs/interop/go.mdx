...
The compiler loads Go packages via **`go/packages`** and type-checks Forst↔Go calls when imports resolve. Supported mappings include primitives, slices, pointers, `error`, and `interface{}` (including variadic calls).
...
<Info>
  ...
  separate](/language/errors-and-result#result-and-tuple-stay-separate) for the
...
**Go import loading** is **experimental**. The compiler loads packages via **`go/packages`** and type-checks calls when imports resolve. Common stdlib paths work; full parity with arbitrary Go packages is still maturing.

**Known mapper gaps:**

- **Generic Go APIs** (`slices.Contains`, `maps.Clone`, …) — no type-argument syntax or inference yet; the compiler reports a clear **`go-call`** diagnostic.
- **`unsafe`** — not wired end-to-end; use hand-written **`.go`**.
- **Non-empty Go interfaces** — named interface types from imports work when the value carries a tracked Go type; Forst shapes do not automatically satisfy interfaces such as **`io.Reader`**.
- **Unnamed struct types** — exported-field anonymous structs map to Forst **shapes** (field access via **`variableGoTypes`**); structs with unexported or embedded fields stay opaque.

**What works today for typical backend libraries:**

- Qualified stdlib calls (`context`, `encoding/json`, `database/sql`, `log/slog`, `net/http`, …) when **`go/packages`** loads the import.
- Named Go types, methods, maps, slices, pointers, and **`error`** at the boundary.
- **Function callbacks** — pass a Forst **`func(...) { ... }`** literal where Go expects a concrete **`func`** type (for example **`http.HandleFunc`**). See **`examples/in/go_interop/http_handle.ft`**.

`switch` / `case` / `fallthrough` are **language** features (Go-faithful), not interop gaps. Type switches (`switch v := x.(type)`) are deliberately unsupported; use **`is`** narrowing instead.

Failed **`go/packages`** loads surface as **`go-import`** diagnostics when the import path is referenced.
