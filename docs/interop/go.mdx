...
Pass a Forst function literal where Go expects a concrete **`func`** type—for example **`http.HandleFunc`**. See **`examples/in/go_interop/http_handle.ft`**.
...
- **Generic Go APIs** (`slices.Contains`, `maps.Clone`, …) — not supported yet; clear **`go-call`** diagnostic.
- **`unsafe`** — not wired end-to-end; use hand-written **`.go`**.
- **Non-empty Go interfaces** — Forst shapes do not automatically satisfy interfaces such as **`io.Reader`**.
- **Anonymous structs** — exported fields map to Forst shapes; embedded or unexported fields stay opaque.
...
<CardGroup cols={2}>
  ...
  <Card title="Call JavaScript from Forst" icon="/icons/typescript.svg" href="/interop/bridge/call-javascript">
