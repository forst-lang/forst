...
[Effect](https://effect.website) is a TypeScript library for programs with typed errors, managed dependencies, retries, cancellation, and safe resource use. It makes these needs visible in your types and gives you tools to combine them.

Forst already gives you typed backend functions and generated clients. Effect mode carries those types into Node. Each generated call has typed Forst failures and declares the service it needs. This makes calls easier to provide, retry, cancel, and replace in tests.
...
See [Call Forst over HTTP](/interop/invoke/call-forst#failures) for the full failure guide.
...
Same three levels as [Testing](/interop/invoke/testing), expressed as layers.
...
**Wire level** with a fake transport, when you need to assert payloads or simulate a specific failure. Provide a mock `ForstTransport` under the real package services. See [Testing](/interop/invoke/testing) for the Promise mode equivalents.
...
For Vitest or Jest hooks, use `makeForstTestServer()` and call `dispose()` in `afterAll`. See [Testing](/interop/invoke/testing#call-a-real-forst-server).
