...
When `server.embedded` or `node.hostMode` is true, `forst dev` compiles and runs your entry `.ft` (same pipeline as `forst run`) instead of the per-invoke HTTP executor.
...
<CardGroup cols={2}>
  ...
  <Card title="Generate a TypeScript client" icon="/icons/typescript.svg" href="/interop/node/generate-types">
    Install, postinstall, and `@forst/gen`.
  </Card>
</CardGroup>
