...
When you invoke `forst` through `@forst/cli`, the CLI downloads official plugin binaries into the same version cache directory as the compiler and prepends that directory to `PATH` before spawning `forst generate`.
