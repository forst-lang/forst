...
| **Go to definition** / **Find references** | Jump across same-package `.ft` files, cross-package Forst imports in the same module, **Node import** bindings (`import "./path" node`) to resolved `.ts`/`.js` files, and **Go imports** to resolved `.go` sources |
...
**Node interop in the editor:** Node import path strings, module aliases, and qualified calls (`payment.create`) support **go to definition** into the resolved TypeScript or JavaScript module. Export names jump to the indexed symbol when the compiler has indexed the module. See [Node interop](/interop/node).
