...
**Node interop in the editor:** JS import path strings, module aliases, and qualified calls (`payment.create`) support **go to definition** into the resolved TypeScript or JavaScript module. Export names jump to the indexed symbol when the compiler has indexed the module. See [Node interop](/interop/node).
