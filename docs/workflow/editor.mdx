...
| **Go to definition** / **Find references** | Jump across same-package `.ft` files, cross-package Forst imports in the same module, **JS import** bindings (`import "./path" node`) to resolved `.ts`/`.js` files, and **Go imports** to resolved `.go` sources |
...
Completion covers keywords, locals, same-package symbols, members on Go-typed bindings, and exported symbols after Go package qualifiers (`pkg.`).

**Go interop in the editor:** qualified imports (`fmt.Println`) and same-package `.go` functions show `go/types` signatures; godoc is included when loads succeed. **Go to definition** jumps into resolved `.go` sources for import paths, package qualifiers (`exec.Command`), same-package exports (`AddInts`), and receiver methods (`cmd.Run`). Cross-package Forst imports (e.g. Providers `auth` → `api`) use module-level typechecking so diagnostics match compile-time arity—not generated Go stubs in the source tree. See [Go interop](/interop/go).

**Node interop in the editor:** JS import path strings, module aliases, and qualified calls (`payment.create`) support **go to definition** into the resolved TypeScript or JavaScript module. Export names jump to the indexed symbol when the compiler has indexed the module. See [JavaScript interop](/interop/invoke).
