...
Use `ensure condition or error` when a failed check should return a known error.
...
- When you know a call has exclusive outcomes, split its values and use
  `ensure !err or err` before returning the success value.
...
`forst generate` and see [Generate client types](/interop/node/generate-types)
...
Error creation through `ensure`, type merging after `or`, and TypeScript `_tag`
output are still maturing. Treat generated payloads as evolving. See
[Generate client types](/interop/node/generate-types#caveats).
...
<CardGroup cols={2}>
  <Card title="Union and intersection types" icon="code-branch" href="/language/union-and-intersection-types">
