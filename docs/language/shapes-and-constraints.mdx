...
| --- | --- | --- |
| **`String`** | `.Min(n)`, `.Max(n)` | Unicode **code point** (rune) bounds |
| | `.MinBytes(n)`, `.MaxBytes(n)` | UTF-8 **byte** length bounds |
...
`String.Min` / `Max` count Unicode code points (`utf8.RuneCountInString`), matching JSON Schema `minLength` / `maxLength`. They are not grapheme clusters and not JavaScript UTF-16 units (Zod `.min` / `.max` still use JS string length). Use `MinBytes` / `MaxBytes` when you need a UTF-8 wire-size cap on text.
...
The typechecker does not fully validate every built-in constraint beyond presence wiring. Prefer explicit constraints or [type guards](/language/type-guards). Names such as `Valid` are available for user-defined type guards.

Generated TypeScript types reflect structure only. Client-side validation is your job unless you call the Forst server. See [Generate client types § Caveats](/interop/invoke/generate-types#caveats).
