...
The most common backend mistake is letting **untrusted input** reach business logic. In Go, you usually declare a struct and write manual checks. In Forst, **constraints live on the type**. The compiler emits runtime validation and rejects invalid data at the boundary.
...
func placeOrder(in: PlaceOrderInput) {
	// in is already validated
}
```

When `placeOrder` runs, the compiler has already proved the shape at compile time where values are known, and emits checks for dynamic input. Invalid SKUs and quantities never reach your catalog logic.
