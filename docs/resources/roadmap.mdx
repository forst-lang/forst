...
| Status | Feature | Notes |
| --- | --- | --- |
| <Icon icon="circle-check" iconType="solid" size={16} color="#16A34A" /> | [`forst generate` (`.d.ts`)](/interop/typescript) | Merged TypeScript declarations from `.ft` sources; shared `ftconfig` discovery with `forst dev`. |
| <Icon icon="circle-check" iconType="solid" size={16} color="#16A34A" /> | [Provider-free client surface](/interop/typescript) | TS emit and sidecar wire stay data-only; functions with unsatisfied `Providers(f)` are omitted from exports. |
| <Icon icon="circle-check" iconType="solid" size={16} color="#16A34A" /> | [`forst dev` HTTP API](/workflow/dev-server-and-sidecar) | `/health`, `/functions`, `/invoke`, `/types`, `/version` for local iteration. |
| <Icon icon="circle-check" iconType="solid" size={16} color="#16A34A" /> | [Sidecar invoke contract](/interop/typescript) | Stable JSON `POST /invoke` from Node/TS clients to a running dev server. |
...
| <Icon icon="flask" iconType="solid" size={16} color="#7C3AED" /> | [Client helper stubs](/interop/typescript) | Thin generated helpers next to `types.d.ts`. |
