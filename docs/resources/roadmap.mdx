...
| <Icon icon="flask" iconType="solid" size={16} color="#7C3AED" /> | [Import and type-check Go packages](/interop/go) | `go/packages` loading and Forst↔Go call checking against loaded Go signatures; common stdlib paths work, not full parity. Failed loads surface as `go-import` diagnostics. Forst sibling imports in the same module resolve from `.ft` sources (not committed Go stubs). |
...
| <Icon icon="flask" iconType="solid" size={16} color="#7C3AED" /> | Go composite types (maps, channels, arrays, funcs) | `map[K]V`, `[N]T`, `chan T`, and `func` types map through the FFI boundary where Forst has matching syntax. Example: `examples/in/go_interop/maps.ft`. Unnamed struct types and Go type aliases still have gaps. |
| <Icon icon="flask" iconType="solid" size={16} color="#7C3AED" /> | [Variadic spread into Go calls](/interop/go#variadic-spread-into-go-calls) | `argv[1:]...` at Go call sites. Example: `examples/in/go_interop/cli.ft` (`task example:go-interop`). |
...
| Status | Feature | Notes |
| --- | --- | --- |
| <Icon icon="circle-check" iconType="solid" size={16} color="#16A34A" /> | [`forst generate` → `@forst/gen`](/interop/node/generate-types) | Linked client package under `.forst/client`; subpath imports per Forst package; shared `ftconfig` discovery with `forst dev`. |
| <Icon icon="circle-check" iconType="solid" size={16} color="#16A34A" /> | [Provider-free client surface](/interop/node/generate-types) | TS emit and invoke wire stay data-only; functions with unsatisfied `Providers(f)` are omitted from exports. |
...
| <Icon icon="circle-check" iconType="solid" size={16} color="#16A34A" /> | [Tagged invoke failures](/interop/node/call-forst) | Stable JSON `POST /invoke` plus `_tag` failures, `.safe()`, and per-call options. |
| <Icon icon="flask" iconType="solid" size={16} color="#7C3AED" /> | [Built-in HTTP server + `@forst/gen`](/interop/node/call-forst) | In-process `/invoke` in compiled Go binaries; generated subpath exports. Pin in production. |
...
| <Icon icon="flask" iconType="solid" size={16} color="#7C3AED" /> | [Testing and Effect mode](/interop/node/testing) | `withForstTestScope`, optional `generate.effect` / `ForstClientLive`. |
