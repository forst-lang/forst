...
| **Runtime checks (`else`)** | `ensure speed is LessThan(100) else TooFast({ ... })`. `else` marks failure; `or` is not for failure. |
...
| **Optional / errors** | `ensure err is Nil() else err`. No `try`/`catch`, no `panic`/`recover`. |
...
| `or` as failure keyword | `else` for typed failure; `or` only for alternative conditions |
