...
- [optionals / `Result`](../optionals/02-result-and-error-types.md) and [generics RFC §10](./00-user-generics-and-type-parameters.md#result-types-generics-and-narrowing-ok-and-err) — **`Result(Success, Failure)`** as a **parameterized** type, **`Ok()`** / **`Err()`** as **type guards**, and **`if` / `ensure`** narrowing.
