...
**Subject:** [SPEC — Function requirements](./SPEC.md) as locked by [ADR-001–017](./ADR.md) (Provider shape literals, inferred `Needs(f)`, mandatory completeness, nested `with`, always-forward scope).
...
| [PL theorist](6a6fad23-fc16-490a-a140-adb755d4a824) | Effects / row types researcher | *ProviderScope DI Without an Environment Algebra* | **6/10** elegance, **5/10** semantic coherence |
...
3. **Always-forward scope** (ADR-004) — call sites hide callee requirements; mitigation is 100% tooling-dependent with no signature anchor (ADR-016 §3).
4. **Fat Provider typo asymmetry** (ADR-012, ADR-017) — missing fields hard-error; unknown/extra fields in CI fixtures may warn-only when no reachable callee needs them.
...
- **PL:** No compositional law tying `Needs(f)`, `Provider`, and `{fn}Needs` — three names, one phenomenon.
...
Go critic: daily ergonomics loss vs one-shot struct literals **partially addressed** by **`t.Run`** table tests in Forst. PL critic: ceremony a row-polymorphic provide would collapse. DX critic: shadow stacks opaque without “effective scope Provider” IDE command.
...
### 3. Always-forward scope hides wiring

ADR-004 + ADR-016 §4: no subtract, pick, or least-privilege. Inner calls inherit full scope; source at `return expireToken(token)` shows **zero** requirement evidence.
...
- **DX:** “Find wiring roots” and effective-scope views are **mandatory** LSP features, not nice-to-haves.

### 4. Provider vs `Needs(f)` — typo safety and namespace split

ADR-017: **Provider** = fixture-shaped wiring (fields = contract idents). ADR-003: **`Needs(f)`** = inferred per function. ADR-012: superset allowed; extras → warning.
...
ADR-013: needs struct fields use **contract types by value**; pointers optional in wiring only. SPEC § Go lowering now matches: `Logger Logger` not `*Logger`; scope → call site via **direct field copies** (`wiring.Logger`), not map indexing.
...
| **Provider shapes** | Adds fixture taxonomy; merge helpers unfinished | Two namespaces for one phenomenon | Parallel type rules on shapes |
...
7. **Provider = shape literals** (ADR-015, ADR-017) — same `{ field: value }` syntax as data shapes; not Go maps; not `with ctx` struct forwarding.
...
**Conditions to reach 7+/10:** Align SPEC emit with ADR-013; table-test-friendly wiring path; documented brownfield `Deps` → Provider migration; pointer vs value locked in SPEC not deferred docs.

### PL theorist — [ProviderScope DI Without an Environment Algebra](6a6fad23-fc16-490a-a140-adb755d4a824)
...
**Minimal fixes:** Lock alias normalization; unknown keys → error; read-only derived `// needs:` annotation; postfix desugar; one normative merge law (`merged = outer ⊕ inner`, keys(scope) ⊇ Needs(callee)).

### Compiler/DX — [Derived Needs, Hidden Wiring](dbeeda4c-a7b1-444a-802d-4cc62831403c)

**Day-one tooling checklist (abbreviated):**

- Derived `Needs(f)` hover distinct from Provider typedef constraints
- Completeness diagnostics with `required by: f → g → h` chains
- Cross-package needs graph = discovery JSON = checker fixed-point
- Effective scope Provider per `with` scope (shadow markers)
...
| Fat Provider typo → warning-only | High | All, DX | Hard error on unknown contract idents |
| No brownfield bridge | High | Go | Document hand-written conversion; revisit after Provider pilot |
| Always-forward hides deps at call site | High | All | Day-one inlay hints + effective-scope IDE |
| Alias key spelling TBD | Medium | PL, DX | Lock root-ident-only; alias quick-fix |
| Provider vs `Needs(f)` conflated in tooling | Medium | PL, DX | Separate LSP fields / hover sections |
