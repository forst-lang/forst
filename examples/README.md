...
- **`in/*.ft`** at the root of `in/` (e.g. `basic.ft`, `go_builtins.ft`, `generics.ft`, `slices.ft`, `method_chain.ft`, `field_after_call.ft`, `string_quote_escape.ft`, `index_then_field.ft`, `bytes_alias.ft`, `ensure.ft`, `nominal_error.ft`, `union_error_types.ft`) are the small, primary examples referenced by the Taskfile and [testing rules](../.cursor/rules/testing.mdc). **`in/method_multireturn/`** unpacks same-package Go method multi-return (`helpers.go` + `main.ft`). **`in/go_interop/`** is the Go FFI + same-package hand-written Go showcase (`cli.ft`, `helpers.go`). Full compile integration for these lives in **`cmd/forst` `TestExamples`**; `internal/compiler` keeps a minimal smoke compile only. Regenerate committed Go goldens under **`out/`** with **`task examples:update-goldens`** (from repo root). That task runs `git add -f examples/out/` so new or updated goldens are staged even though the root `out` ignore rule would otherwise hide them from `git status`.
...
| **Guards** | [in/rfc/guard/guard.md](in/rfc/guard/guard.md) (no folder README) | Partially realized | Shape guards and `ensure` are in the language; [anonymous_objects.md](in/rfc/guard/anonymous_objects.md), [interop.md](in/rfc/guard/interop.md) extend the story. **Disjunction / enums / the refinement trap:** [in/rfc/refinements/README.md](in/rfc/refinements/README.md). |
| **Analyzable refinements** | [in/rfc/refinements/README.md](in/rfc/refinements/README.md) | Specification / design · **accepted** | [12](in/rfc/refinements/12-accepted-decision.md) is `ensure` / assertion `or` / typed `else` / type `|` / guards. [14](in/rfc/refinements/14-type-targets.md) is type vs assertion on the RHS of `is`. [13](in/rfc/refinements/13-refinement-stability.md) is mutation / aliasing / Go: facts die on overlapping writes, mutation stays legal. Plan: [.plans/analyzable-refinements](../.plans/analyzable-refinements/README.md). |
...
| **`switch` vs `match`** | [in/rfc/switch-match/README.md](in/rfc/switch-match/README.md) | Specification / design · **decision recorded** | Resolves `ROADMAP.md`'s open `switch` design question: Go-faithful `switch` now, `match` deferred to its own future RFC. See [00-switch-and-match.md](in/rfc/switch-match/00-switch-and-match.md). |
