...
function openTelemetryRequestedFromEnv(): boolean {
  if (process.env.FORST_BRIDGE_OTEL?.trim() === "1") {
    return true;
  }
  ...
  return Effect.gen(function* () {
    ...
    if (otelModule === null) {
      yield* Effect.logWarning(
        "OpenTelemetry requested (FORST_BRIDGE_OTEL or OTEL_EXPORTER_OTLP_ENDPOINT) but @forst/runtime/opentelemetry peers are not installed; continuing with stderr spans only"
...
/** Alias for {@link resolveForstNodeRuntimeLayer}. */
export const resolveForstRuntimeLayer = resolveForstNodeRuntimeLayer;
