...
import {
  ...
  type ForstRuntimeLayerOptions,
} from "./layer.js";

/** Environment variable to opt in to OpenTelemetry when no OTLP endpoint is set. */
export const envOpenTelemetry = "FORST_BRIDGE_OTEL";
...
const defaultServiceName = "forst-runtime";
...
/** Returns true when OTEL should activate (`FORST_BRIDGE_OTEL=1` or OTLP endpoint set). */
...
export function mergeForstNodeRuntimeWithOpenTelemetry(
  ...
  forstOptions?: ForstRuntimeLayerOptions
): Layer.Layer<never> {
  return Layer.mergeAll(
    makeForstRuntimeLayer(forstOptions),
    otelLayer
