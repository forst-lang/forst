...
process.stderr.write(
  `${JSON.stringify({
    ...
    message:
      "register.cjs is deprecated; use @forst/runtime/host/register.mjs with NODE_OPTIONS --import",
  })}\n`
);
...
void Promise.all([import("../host.js"), import("effect"), import("../effect/layer.js")])
  .then(async ([hostMod, effectMod, layerMod]) => {
    ...
    const appReadyModule = process.env.FORST_BRIDGE_APP_READY_MODULE?.trim();
