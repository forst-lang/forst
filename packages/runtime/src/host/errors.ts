...
/** Raised when `FORST_BRIDGE_SOCKET` is unset on Unix. */
export class HostSocketRequiredError extends HostSocketRequiredErrorBase<{
  readonly message: "FORST_BRIDGE_SOCKET is required on Unix";
}> {}
...
export function hostSocketRequired(): HostSocketRequiredError {
  return new HostSocketRequiredError({ message: "FORST_BRIDGE_SOCKET is required on Unix" });
}
