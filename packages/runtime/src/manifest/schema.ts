...
export const FORST_BRIDGE_MANIFEST_V1_VERSION = 1 as const;
...
export const FORST_BRIDGE_EXPORT_KINDS: readonly ForstNodeExportKind[] = [
  "function",
  "asyncFunction",
  "generator",
  "asyncGenerator",
...
export interface ForstNodeManifestV1 {
  /** Manifest schema version; must be {@link FORST_BRIDGE_MANIFEST_V1_VERSION}. */
  version: typeof FORST_BRIDGE_MANIFEST_V1_VERSION;
...
export function isForstNodeExportKind(value: unknown): value is ForstNodeExportKind {
  return (
    typeof value === "string" &&
    (FORST_BRIDGE_EXPORT_KINDS as readonly string[]).includes(value)
  );
}
...
function parseExportKind(value: unknown, path: string): ForstNodeExportKind {
  ...
  if (!isForstNodeExportKind(kind)) {
    throw new ForstNodeSchemaValidationError(
      path,
      `expected one of ${FORST_BRIDGE_EXPORT_KINDS.join(", ")}`,
    );
  }
  return kind;
}
...
export function parseForstNodeManifestV1(value: unknown): ForstNodeManifestV1 {
  ...
  if (version !== FORST_BRIDGE_MANIFEST_V1_VERSION) {
    throw new ForstNodeSchemaValidationError(
      "manifest.version",
      `expected ${FORST_BRIDGE_MANIFEST_V1_VERSION}`,
    );
  }
  ...
  return {
    version: FORST_BRIDGE_MANIFEST_V1_VERSION,
    boundaryRoot,
