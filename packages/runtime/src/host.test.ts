...
describe("startForstNodeHost", () => {
  const prevHost = process.env.FORST_BRIDGE_HOST;
  const prevLeader = process.env.FORST_BRIDGE_HOST_LEADER;
  const prevSocket = process.env.FORST_BRIDGE_SOCKET;
  const prevReady = process.env.FORST_BRIDGE_HOST_READY;

  beforeEach(() => {
    process.env.FORST_BRIDGE_HOST = "1";
    process.env.FORST_BRIDGE_HOST_LEADER = "1";
  ...
  afterEach(() => {
    ...
    if (prevHost === undefined) {
      delete process.env.FORST_BRIDGE_HOST;
    } else {
      process.env.FORST_BRIDGE_HOST = prevHost;
    }
    if (prevLeader === undefined) {
      delete process.env.FORST_BRIDGE_HOST_LEADER;
    } else {
      process.env.FORST_BRIDGE_HOST_LEADER = prevLeader;
    }
    if (prevSocket === undefined) {
      delete process.env.FORST_BRIDGE_SOCKET;
    } else {
      process.env.FORST_BRIDGE_SOCKET = prevSocket;
    }
    if (prevReady === undefined) {
      delete process.env.FORST_BRIDGE_HOST_READY;
    } else {
      process.env.FORST_BRIDGE_HOST_READY = prevReady;
    }
  });

  test("noop when FORST_BRIDGE_HOST unset", async () => {
    delete process.env.FORST_BRIDGE_HOST;
  ...
  test("noop when FORST_BRIDGE_HOST_LEADER unset and register not preloaded", async () => {
    delete process.env.FORST_BRIDGE_HOST_LEADER;
  ...
  test("idempotent start returns same promise", async () => {
    ...
    process.env.FORST_BRIDGE_SOCKET = socketPath;
    process.env.FORST_BRIDGE_HOST_READY = readyPath;
  ...
  test("fails when FORST_BRIDGE_SOCKET unset on Unix", async () => {
    if (process.platform === "win32") {
      return;
    }
    delete process.env.FORST_BRIDGE_SOCKET;
    ...
    if (Either.isLeft(result)) {
      expect(String(result.left)).toContain("FORST_BRIDGE_SOCKET is required on Unix");
    }
  });
