 ...
 * when `FORST_BRIDGE_HOST=1`, with optional deferred app-ready signaling.
 *
...
const envHostEnabled = "FORST_BRIDGE_HOST";
const envHostLeader = "FORST_BRIDGE_HOST_LEADER";
...
const hostStartNonLeaderNoop = Effect.fn("Host.startNonLeaderNoop")(function* () {
  yield* Effect.annotateCurrentSpan("reason", "FORST_BRIDGE_HOST_LEADER unset");
...
const hostStartNoop = Effect.fn("Host.startNoop")(function* () {
  yield* Effect.annotateCurrentSpan("reason", "FORST_BRIDGE_HOST unset");
  return {
    socketPath: "",
    close: () => Effect.void,
  } satisfies HostHandle;
...
/**
 * No-op unless FORST_BRIDGE_HOST=1. Idempotent. Logs to stderr only. Single client connection.
 */
