...
describe("printForstCliVersion", () => {
  test.skipIf(process.platform === "win32")(
    "prints npm semver and first line of forst version",
    async () => {
      const dir = mkdtempSync(join(tmpdir(), "forst-cli-ver-"));
      const fake = join(dir, "forst");
      const script = `#!/bin/sh
if [ "$1" = "version" ]; then
  echo "forst 9.9.9 abc"
  echo "extra line"
fi
`;
      writeFileSync(fake, script);
