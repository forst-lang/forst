...
test("docs/forst-highlight.js is generated by sync:grammar", () => {
  const highlightPath = path.join(__dirname, "..", "..", "..", "docs", "forst-highlight.js");
  const js = fs.readFileSync(highlightPath, "utf8");
  assert.match(js, /generated by sync-forst-grammar/);
...
test("call site: placeOrder({ is one function name", async () => {
  const line = "x := placeOrder({";
  await assertScopeOn(line, "placeOrder", "entity.name.function");
  await assertScopeOn(line, "place", "entity.name.function");
});
