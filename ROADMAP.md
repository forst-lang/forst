...
| Variadic spread in Go call arguments (`expr[low:]...`) | 🔬 experimental | Spread a Forst slice subslice into a Go variadic parameter at call sites (e.g. `exec.Command(argv[0], argv[1:]...)`). Checked and lowered via Go FFI (`go_interop.go`, transformer). Example: `examples/in/go_interop.ft` (`task example:go-interop`). |
| Field and method access on Go values | 🔬 experimental | After a Go import binding or qualified call, dotted field paths and postfix method chains on Go-typed receivers (e.g. `cmd.ProcessState.ExitCode()`). Checked via `go/types` when the import package loads. Example: `examples/in/go_interop.ft`. |
