...
| `Result(Success, Failure)` + structured `error` | 🔬 experimental | **`Result(S, F)`** exists end-to-end with **`F`** constrained to the **`Error`** family today: built-in type (`forst/internal/ast/type.go`, `result_expr.go`), inference (`inferred_storage.go`, `typechecker_test.go`), **`ensure`** + **`if x is Ok()`** narrowing (`infer_if.go` / related), and Go interop that folds **`(T, error)`** into **`Result(T, Error)`** (`go_interop.go`). **`Ok`/`Err`** parse as expressions but are **not** general **`Result`** value constructors yet—they serve as **`is`/`ensure`** discriminants (see `result_expr.go` comments). Examples: `examples/in/result_ensure.ft`, `examples/in/result_if.ft` (`task example:result-ensure`, `task example:result-if`). **Gaps vs “done”:** arbitrary non-**`Error`** failure types, user **generics** on **`Result`**, **RFC 02** ensure-only failure diagnostics + implicit **`error N`** + **`F`** LUB / export rules, and polish across all compiler paths—see [Result & error types](./examples/in/rfc/optionals/02-result-and-error-types.md), [02 — first-class errors](./examples/in/rfc/errors/02-first-class-errors-normative.md), and [generics ↔ `Result`](./examples/in/rfc/generics/00-user-generics-and-type-parameters.md#result-types-generics-and-narrowing-ok-and-err). |
