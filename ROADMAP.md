...
| `forst test` (Go-native `Test*` in `*_test.ft`) | 🔬 experimental | Discovers `Test*` functions, emits `*_test.go`, runs `go test`. Primary use today: Providers wiring in tests (`with` at test entry points); command spec still draft ([forst-test RFC hub](./examples/in/rfc/forst-test/README.md)). `task test:providers`, `task test:providers-cross_pkg`. |
