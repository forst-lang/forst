...
| Go FFI expressions (subslice, spread, field/method chains) | 🔬 experimental | Subslices `xs[low:high]` / `xs[low:]` / `xs[:high]`, variadic spread `xs[low:]...`, and field/method access on Go-typed receivers (e.g. `cmd.ProcessState.ExitCode()`). LSP preloads stdlib import packages for hover/completion. Examples: `examples/in/slices.ft`, `examples/in/go_interop.ft`. |
