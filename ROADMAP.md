...
| Type-check Forst↔Go calls | 🔬 experimental | Qualified calls **`pkg.Func`** are checked against loaded Go signatures when imports resolve. Primitives, slices, pointers, `error`, and `interface{}` (incl. variadic) are mapped; named Go types, maps, channels, fixed arrays, and function types map through the FFI boundary. Numeric kinds and interface assignability use **`go/types`** where wired. **Still open:** generic Go API instantiation, full callback typing, and unnamed struct / Go type-alias gaps at the boundary. |
...
| Go composite types (maps, channels, arrays, funcs) | 🔬 experimental | `map[K]V`, `[N]T`, `chan T`, and `func` types map through the FFI boundary where Forst has matching type syntax. Unnamed struct types and Go type aliases still have gaps. |
