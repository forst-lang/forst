...
| Type-check Forst↔Go calls | 🔬 experimental | Qualified calls **`pkg.Func`** are checked against loaded Go signatures when imports resolve. Primitives, slices, pointers, `error`, and `interface{}` (incl. variadic) are mapped; named Go types, maps, channels, fixed arrays, and function types are improving. **Still open:** numeric-kind precision at the boundary, sound interface assignability, generic Go API instantiation, and full callback typing. |
