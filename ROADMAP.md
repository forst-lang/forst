...
| NPM package (installable compiler / CLI) | 🔬 experimental | [`@forst/cli`](./packages/cli/README.md): `npx @forst/cli` / `node_modules/.bin/forst` downloads the pinned native binary from GitHub Releases; published with compiler tags via [release.yml](./.github/workflows/release.yml). Distinct from sidecar. |
| Run compiler or sidecar from Node.js | 🔬 experimental | Sidecar: `npm install @forst/sidecar` when published (JSR: `npx jsr add @forst/sidecar`); compiler path via [`@forst/cli`](./packages/cli/README.md) [`resolveForstBinary`](./packages/cli/src/resolve.ts) / [`ensureCompiler`](./packages/sidecar/src/utils.ts). Monorepo dev path unchanged. |
