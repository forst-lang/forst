...
| Same-package unexported Go symbols | 🔬 experimental | Mixed `.ft` + `.go` in one package can call unexported Go funcs and access unexported fields when the Go source is co-located. **`forst test`** sandboxes link sibling **`.go`** (and shim Go-only deps). **`forst run`** / **`watch`** emit beside the entry package when **`generate.go`** (or **`-o`**) is set so handwritten same-package Go is included; plain ad-hoc runs stay sandboxed. Packages without embedded invoke build as a normal package directory. |
...
| `forst test` (Go-native `Test*` in `*_test.ft`) | 🔬 experimental | Finds **`Test*`** functions in **`*_test.ft`**, generates temporary Go test code, and runs **`go test`**. Walks **every** package path argument; sandboxes include sibling **`.go`** and Go-only deps. Primary use: Providers wiring, mixed Forst+Go packages, and cross-package integration. Docs: [CLI — `forst test`](./docs/workflow/cli.mdx), [forst-test RFC hub](./examples/in/rfc/forst-test/README.md). |
